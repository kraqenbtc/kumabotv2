...
const WS_PORT = 8082; // ETH:8080, BTC:8081
...
const HTTP_PORT = 3002; // ETH:3000, BTC:3001
...
const tradeHistory = new TradeHistory('SOL-USD');
...
function log(type, message, data = null) {
  ...
  if (data) {
    console.log(`[${time}][SOL][${type}] ${message}`, data);
  } else {
    console.log(`[${time}][SOL][${type}] ${message}`);
  }
}
...
function setupWebSockets() {
  // Orderbook
  state.wsOrderbook = new WebSocket(`${config.wsUrl}/SOL-USD@l1orderbook`);
